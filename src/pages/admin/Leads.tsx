...
setLeads(leads.map(l => l.id === id ? {...l,important:!important} : l))
await supabase.from("leads").update({important:!important}).eq("id",id)
}
...
const { message } = parseMessage(lead.message)

const text = `Hola ${lead.name}! Vi tu consulta: "${message}".`
...
setNewLeadIds(prev => prev.filter(l => l !== id))
}
...
isNew ? "bg-green-50 ring-2 ring-green-300" : "hover:bg-gray-50"
}`}
onMouseEnter={()=> isNew && markAsSeen(lead.id)}
>

<td className="p-4">
<button onClick={()=>toggleImportant(lead.id,lead.important)}>
<Star size={18} className={lead.important ? "text-yellow-500" : "text-gray-300"} fill={lead.important ? "currentColor" : "none"}/>
</button>
</td>
...
{isNew && <span className="ml-2 text-xs bg-green-500 text-white px-2 py-1 rounded">Nuevo</span>}
</td>

<td>
<a href={`mailto:${lead.email}`} className="flex items-center gap-2 hover:text-black">
<Mail size={14}/>
{lead.email}
</a>
</td>
...
<td className="max-w-[250px] truncate text-gray-600 text-xs">
{parsed.message}
</td>
...
{isNew ? "Nuevo" : timeAgo(lead.created_at)}
</td>
